...
class TestCitibikeDatabase(unittest.TestCase):
	...
	def test_park_bike(self):
		...
		self.assertEquals(station['availableDocks'], 12)
		self.assertEquals(self.db.park_bike(999999), 'error')
		
	def test_rent_bike(self):
		self.reset_data()
		self.db.rent_bike(281)
		station = self.db.get_station(281)
		self.assertEquals(station['availableBikes'], 35)
		self.assertEquals(station['availableDocks'], 14)
		self.assertEquals(self.db.park_bike(999999), 'error')
	...
	def test_get_closest_station(self):
		self.reset_data()
		closestStation = self.db.get_closest_station(40.7643971, -73.97371465)
